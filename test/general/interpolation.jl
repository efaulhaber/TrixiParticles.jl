@testset verbose = true "SPH Interpolation" begin
    function compare_interpolation_result(actual, expected; tolerance = 1.0e-8)
    ...
    function binary_search_outside(start, end_, func = nothing; tolerance = 1.0e-5)
    ...
    @testset verbose = true "2D" begin
        ...
        # some simple results
        expected_zero(y) = (
            density = NaN, neighbor_count = 0, coord = [0.0, y],
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                @test isapprox(distance_top_outside, 0.11817321777343714, atol = 1.0e-14)
                ...
                @test isapprox(distance_bottom_outside, 0.1181732177734375, atol = 1.0e-14)
                ...
                result_multipoint = TrixiParticles.interpolate_point(
                    multi_point_coords,
                    semi_no_boundary,
                    fluid_system,
                    v_no_bnd, u_no_bnd,
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                if cut_off_bnd
                    @test isapprox(
                        distance_bottom_outside, 3.637978807091713e-13,
                        atol = 1.0e-14
                    )
                else
                    @test isapprox(distance_bottom_outside, 0.11817145975473978, atol = 1.0e-14)
                end
    ...
    @testset verbose = true "3D" begin
        ...
        # some simple results
        expected_zero(y) = (
            density = NaN, neighbor_count = 0, coord = [0.0, y, 0.0],
        ...
        for cut_off_bnd in [true, false]
            @testset verbose = true "Interpolation Point no boundary - cut_off_bnd = $(cut_off_bnd)" begin
                ...
                @test isapprox(distance_top_outside, 0.09390581548213905, atol = 1.0e-14)
                ...
                @test isapprox(distance_bottom_outside, 0.09390581390689477, atol = 1.0e-14)
