...
function interact!(dv, v_particle_system, u_particle_system,
    ...
    foreach_point_neighbor(particle_system, neighbor_system,
        ...
        for i in 1:ndims(particle_system)
            @inbounds dv[particle, i] += dv_particle[i]
            # Debug example
        ...
        @inbounds continuity_equation!(dv, density_calculator, particle_system,
                                       neighbor_system, v_a, v_b, particle, neighbor,
...
function interact!(dv, v_particle_system, u_particle_system,
                   v_neighbor_system, u_neighbor_system,
                   particle_system::WeaklyCompressibleSPHSystem, neighbor_system::WeaklyCompressibleSPHSystem, semi)
    (; density_calculator, correction) = particle_system
    ...
    @boundscheck checkbounds(system_coords, particles, ndims(particle_system))

    # Loop over all pairs of particles and neighbors within the kernel cutoff
    @threaded semi for particle in particles
        # Now we can safely assume that `point` is inbounds
        # rho_a = @inbounds current_density(v_particle_system, particle_system, particle)
        m_a = @inbounds hydrodynamic_mass(particle_system, particle)
        p_a = @inbounds current_pressure(v_particle_system, particle_system, particle)
        # This returns density zero?
        # vrho_a = @inbounds extract_svector(v_particle_system, particle_system, particle)
        # v_a = vrho_a[SVector(1, 2)]
        # rho_a = vrho_a[end]
        ...
        @inbounds PointNeighbors.foreach_neighbor(system_coords, neighbor_system_coords,
                                   neighborhood_search, particle) do particle, neighbor,
                                                                      pos_diff, distance

            # vrho_b = @inbounds extract_svector(v_neighbor_system, neighbor_system, neighbor)
            # v_b = vrho_b[SVector(1, 2)]
            # rho_b = vrho_b[end]
            v_b = @inbounds current_velocity(v_neighbor_system, neighbor_system, neighbor)
            rho_b = @inbounds current_density(v_neighbor_system, neighbor_system, neighbor)
